...
unsigned int nStakeMinAge = 60 * 60 * 24;	// minimum age for coin age: 24hrs
unsigned int nStakeMaxAge = 60 * 60 * 24 * 15;	// stake age of full weight: 30d
int64 nStakeTargetSpacing = 60;			// 1-minute block spacing
int64 nWorkTargetSpacing = 60;			// 1-minute block spacing

static const int64 POW_RESTART = 0; // When (block) to unstuck PoW

int64 totalCoin = -1;
int64 nChainStartTime = 1422271700;
...
float GetProofOfWorkReward(int nHeight, int64 nFees, uint256 prevHash)
{
...
{
	int64 nRewardCoinYear = 50;
	
	nRewardCoinYear >>= (nHeight / 525600);
...
static const int64 nTargetTimespan = 60;  // 1 min
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    bool fEnforceBIP30 = true; // Always active in GreenCoin
    bool fStrictPayToScriptHash = true; // Always active in GreenCoin
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "GreenCoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // Genesis block
        const char* pszTimestamp = "19/01/2015: Australian Open Commences";
        CTransaction txNew;
        ...
        txNew.strTxComment = "text:GreenCoin genesis block";
        ...
        assert(block.hashMerkleRoot == uint256("0x689b98ef98e600955e7315cd53e0903eae25e263ad67b7bf9487cc1c97b8d71d"));
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv)
{
    ...
    if (strCommand == "version")
    {
        ...
        // GreenCoin: disconnect any known version prior 2.0.3
        // as these have PoS not working and could serve us garbage
        printf("connected subver %s\n", pfrom->strSubVer.c_str());
        if (!strcmp(pfrom->strSubVer.c_str(),"/GreenCoin:2.0.1/")
            || !strcmp(pfrom->strSubVer.c_str(),"/GreenCoin:2.0.2.1/")
            || !strcmp(pfrom->strSubVer.c_str(),"/GreenCoin:0.7.2/")
   ...
   if(totalCoin > VALUE_CHANGE)
    ...
    int64 nFees = 0;
    {
        ...
        if (pblock->IsProofOfWork())
        {
            ...
            if(totalCoin > VALUE_CHANGE)
                pblock->vtx[0].vout[1].nValue = GetContributionAmount(pindexPrev->nHeight+1);
        }
...
// GreenCoin Foundation Reward

float GetContributionAmount(int nHeight) {
   
	float nBlockReward = 3.8;
	
	float nPOSReward = nBlockReward;
